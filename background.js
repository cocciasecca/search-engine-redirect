...
async function generateAndApplyRules() {
    const items = await chrome.storage.sync.get(['searchEngineUrl', 'additionalOptions']);
    const searchEngineUrl = items.searchEngineUrl || "https://search.brave.com/search";
    const additionalOptions = items.additionalOptions || "";

    const newRules = [];
    let ruleId = 1;

    GOOGLE_DOMAINS.forEach(domain => {
        // if search engine URL contains a "?" already we gotta add options with "&" otherwise the first option will be with "?" and the following one with "&"
        const separator = searchEngineUrl.includes('?') ? '&' : '?';
        ...
        newRules.push({
            ...
            action: {
                type: "redirect",
                redirect: {
                    regexSubstitution: `${searchEngineUrl}${separator}q=\\1${additionalOptions}`
                }
            },
            condition: {
                regexFilter: `${domain.replace(/\./g, '\\.')}/search\\?.*?[&?]q=([^&#]+).*`,
...
chrome.storage.onChanged.addListener((changes, namespace) => {
    if (namespace === 'sync' && (changes.searchEngineUrl || changes.additionalOptions)) {
        generateAndApplyRules();
    }
});